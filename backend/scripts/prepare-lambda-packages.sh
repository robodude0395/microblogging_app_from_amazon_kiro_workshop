...
prepare_package() {
    ...
    if grep -q "withAuth" $function_path; then
        ...
        cp ./dist/common/middleware.js $package_dir/
...
for func in ./dist/functions/auth/*.js; do
    [ -f "$func" ] || continue
    prepare_package $func
done

# Prepare packages for user functions
for func in ./dist/functions/users/*.js; do
    [ -f "$func" ] || continue
    prepare_package $func
done

# Prepare packages for post functions
for func in ./dist/functions/posts/*.js; do
    [ -f "$func" ] || continue
    prepare_package $func
done

# Prepare packages for comment functions
for func in ./dist/functions/comments/*.js; do
    [ -f "$func" ] || continue
    prepare_package $func
done

# Prepare packages for monitoring functions
for func in ./dist/functions/monitoring/*.js; do
    [ -f "$func" ] || continue
    prepare_package $func
done
